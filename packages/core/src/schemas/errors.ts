...
const ERROR_CONFIG: Record<APIErrorCode, { status: number; message: string }> = {
  ...
  [APIErrorCode.SESSION_EXPIRED]: { status: 401, message: 'Session has expired' },
  ...
  [APIErrorCode.INVALID_INPUT]: { status: 400, message: 'Invalid input provided' },
  [APIErrorCode.UNAUTHORIZED]: { status: 403, message: 'Unauthorized access' },
